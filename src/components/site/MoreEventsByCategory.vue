      ...
      <div class="container mx-auto px-6">
        <div class="">
          <h3
            ...
            v-if="category && events.length > 1"
          >
            More events By {{ category.title }}
          </h3>
          <!--  -->
          <div
            ...
            <div v-for="event in events" :key="event.index">
              ...
              >
                ...
                <div class="px-5 py-3 mt-2">
                  ...
                  <h3 v-if="event.category" class="text-xs font-medium text-blue-600 uppercase mt-2">
                    {{ event.category.title }}
                  </h3>
                  <span class="text-gray-500 mt-2">u$s {{ event.price }}</span>
                </div>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      single(id) {
        this.$router.push({ name: 'site-event', params: { id } });
      },
      async getEvents() {
        try {
          if (!this.category) {
            ...
            this.events = response.data.filter(
              (evento) => evento._id !== this.$route.params.id,
            );
